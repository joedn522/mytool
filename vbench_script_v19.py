...
def run_vbench(mp4, dim, odir):
    ...
    env = os.environ.copy()
    env["MASTER_PORT"] = str(get_free_port())   # ★ 每次用不同埠
...
def main():
    ...
    with Manager() as m:
        ...
        workers = [Process(target=consumer, args=(q, results, DBG_FILE, len(tasks)))
                   for _ in range(args.max_video_processes)]  # Pass total_tasks here
        ...
        with open(OUT_FILE, "w", newline="") as f:
            csv.DictWriter(f, ["videoid","Imgurl"]+VBENCH_DIMS, delimiter="\t").writerows(rows)
