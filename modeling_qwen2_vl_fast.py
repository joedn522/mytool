...
if is_flash_attn_2_available():
    ...
    # ── block-wise SDPA：一次只跑 ≤BLOCK 個 Query，降低峰值顯存 ──
    def _flash_attention_forward(q, k, v, *_, is_causal=True, **__):
        # q,k,v: (B,Q,H,D)
        B, Q, H, D = q.shape
        BLOCK = 512        # ← 視顯存再調小；256 更省、速度略慢
        ...
        for s in range(0, Q, BLOCK):
            ...
            qs  = q[..., s:e, :]                          # (B,H,b,D)
            # (B,H,b,Q)
            scores = torch.matmul(qs.float(), k.float().transpose(-2, -1)) * scale
            if is_causal:
                causal = torch.triu(
                    torch.ones(e - s, Q, device=scores.device, dtype=torch.bool), 1 + s
                )
                scores = scores.masked_fill(causal, float('-inf'))
            attn = torch.softmax(scores, dim=-1, dtype=torch.float32).to(q.dtype)
            outs.append(torch.matmul(attn, v))            # (B,H,b,D)

        out = torch.cat(outs, dim=-2)                     # (B,H,Q,D)
        return out.permute(0, 2, 1, 3).reshape(B, Q, H * D)

    def flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k,
                               max_seqlen_q, max_seqlen_k,
                               dropout_p=0.0, causal=True, **__):
        chunks, st = [], 0
        for ed in cu_seqlens_q[1:]:
            qs, ks, vs = q[st:ed], k[st:ed], v[st:ed]               # (L,H,D)
            scale = qs.size(-1) ** -0.5
            s = torch.matmul(qs.permute(1,0,2).float(),
                             ks.permute(1,0,2).float().transpose(-2,-1)) * scale
            if causal:
                m = torch.triu(torch.ones_like(s[0]), 1).bool()
                s = s.masked_fill(m, float('-inf'))
            w = torch.softmax(s, dim=-1, dtype=torch.float32).to(qs.dtype)
            out = torch.matmul(w, vs.permute(1,0,2)).permute(1,0,2)             # (L,H,D)
            chunks.append(out)
            st = ed
...
QWEN2_VL_VISION_ATTENTION_CLASSES = {
    ...
    "flash_attention_2": VisionFlashAttention2
        if flash_attn_varlen_func is not None else VisionAttention,
}
...
QWEN2_VL_ATTENTION_CLASSES = {
    "flash_attention_2": Qwen2VLFlashAttention2
        if flash_attn_varlen_func is not None else Qwen2VLAttention,
}
...
class Qwen2VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2VLConfig, layer_idx: int):
        ...
        # 如果實際沒有 flash-attn，降級但只給 warning
        if flash_attn_varlen_func is None and config.attn_implementation == "flash_attention_2":
            logger.warning("Flash-Attention 未安裝，已自動 fallback 至 PyTorch SDPA。")
