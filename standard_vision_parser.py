...
class VisionParser:
    def __init__(
        ...
        self.video_sampling_strategy = video_sampling_strategy or {}
        # 預設：推論時（非訓練）強制用中間 5 秒；可被上游覆寫
        self._center_clip = self.video_sampling_strategy.get("center_clip", True)
        self._center_sec  = self.video_sampling_strategy.get("clip_len_sec", 5)
        print(f"[VisionParser] center_clip={self._center_clip}, clip_len_sec={self._center_sec}")
    ...
    def _pick_path(self, v: Union[str, Dict]) -> str:
        if isinstance(v, str):
            return v
    ...
    def _ffprobe_duration(self, path: str) -> Union[float, None]:
        ...
        try:
            cmd = [
                "ffprobe", "-v", "error",
                "-select_streams", "v:0",
                "-show_entries", "format=duration,streams=nb_frames,avg_frame_rate",
                "-of", "json", path,
            ]
            out = subprocess.check_output(cmd, stderr=subprocess.STDOUT)
            meta = json.loads(out.decode("utf-8"))
            # 1) prefer container duration
            dur = None
            if "format" in meta and "duration" in meta["format"]:
                try:
                    ...
                    dur = None
            # 2) fall back: nb_frames / avg_frame_rate
            if dur is None and "streams" in meta and meta["streams"]:
                s = meta["streams"][0]
                nb = s.get("nb_frames")
                fr = s.get("avg_frame_rate")
                if nb and fr and fr != "0/0":
                    try:
                        num, den = fr.split("/")
    ...
    def _center_window(self, video_path: Union[str, Dict], sec: float):
        ...
        if dur is None or dur <= 0:
            ...
            return None
        start = max(0.0, (dur - sec) / 2.0)
        end   = min(dur, start + sec)
        print(f"[VisionParser] Extracting center window: start={start:.2f}s, end={end:.2f}s, duration={dur:.2f}s, sec={sec}")
        return start, end
    ...
    def load_video_item(self, video_item) -> List[Image.Image]:
        ...
        # 若上游沒有明確指定 frame/time 範圍，推論時強制取「中間 5 秒」
        wants_center = (
            self._center_clip
            and not self.is_training
            and frame_indices is None and time_indices is None
            and start_frame is None and end_frame is None
            and start_time is None and end_time is None
        )
        print(f"[VisionParser] load_video_item: wants_center={wants_center}, center_clip={self._center_clip}, is_training={self.is_training}")
        if wants_center:
            se = self._center_window(video_path, self._center_sec)
            if se is not None:
                start_time, end_time = se
                print(f"[VisionParser] Using center {self._center_sec}s: start_time={start_time}, end_time={end_time}")
        ...
        print(f"[VisionParser] sample_video: n_frames param={n_frames}, actually extracted={len(frames)} frames")
