...
def consumer(q: Queue, results, dbg, total_tasks):
    ...
    while True:
        ...
        if not mp4:
            ...
            results.append({"videoid": vid, "Imgurl": vurl, "camera_motion": "convert_failed"})
            continue

        row = {"videoid": vid, "Imgurl": vurl}
        base_out = os.path.join(args.output_path, "evaluate_result")
        video_start_time = time.time()  # 記錄單部影片處理開始時間
        odir = os.path.join(base_out, "camera_motion", vid)  # 每影片專屬子目錄
        os.makedirs(odir, exist_ok=True)
        predict_types_str, elapsed, err_tag, stderr_txt = run_vbench(mp4, odir)
        row["camera_motion"] = predict_types_str if not err_tag else err_tag
        if err_tag:  # 不論何種錯誤都寫 tag + 簡短 stderr
            log(f"{vid}\tcamera_motion\t{err_tag}\t{stderr_txt}")
        else:
            log(f"{vid}\tcamera_motion:{predict_types_str}\t{elapsed:.2f}s")
        video_elapsed_time = time.time() - video_start_time  # 單部影片處理時間
        results.append(row)

        # 即時將結果寫入 OUT_FILE
        with open(OUT_FILE, "a", newline="") as f:
            writer = csv.DictWriter(f, ["videoid", "Imgurl", "camera_motion"], delimiter="\t")
            writer.writerow(row)
        ...
        print(f"[RESULT] {vid} - camera_motion: {row['camera_motion']}", flush=True)
